...
/**
 * Resizable-array dequeue for boxed (Object) items.
 */
public class ObjVec {
    /**
    ...
    /** Create a new object vector containing a single value */
    public static ObjVec FromValue(Object v){
     ...
     */
    public ObjVec(Object[] c) {
        allocateElements(c.length);
        for (Object d : c) addLast(d);
    }

    /**
     * Create a copy of 'other'. No data is shared.
     */
    public ObjVec(ObjVec other){
    ...
    /**
     * Remove and return the item at the end of this deque
     * @throws NoSuchElementException {@inheritDoc}
     */
     ...
     */
    public ObjVec slice(int start, int end) {
        if (start < 0) start += length();
        if (end < 0) end += length();
        if (start < 0 || start >= end) return new ObjVec();

        ObjVec result = new ObjVec(end - start);
